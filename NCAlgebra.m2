...
NCRingMap RingElement := (f,x) -> (
   if class x === QQ or class x === ZZ then return promote(x, target f);
   if ring x =!= source f then error "Ring element not in source of ring map.";
   C := ring x;
...
NCRingMap _ ZZ := (f,n) -> (
   ...
   B := source f;
   C := target f;
   --if coefficientRing B != coefficientRing C then error "Expected rings defined over the same coefficient ring.";
   --kk := coefficientRing B;
   srcBasis := flatten entries basis(n,B);
   tarBasis := flatten entries basis(n,C);
   local retVal;
   if #srcBasis == 0 or #tarBasis == 0 then retVal = map((coefficientRing B)^(#tarBasis),(coefficientRing C)^(#srcBasis),0)
   else (
...
NCMatrix _ ZZ := (M,d) -> (
   ...
   multTable := applyTable(entryTable, e -> leftMultiplicationMap((entries M)#(e#0)#(e#1), 
	                                                d - (M.source)#(e#1),
                                                        d - (M.target)#(e#0)));
   matrix multTable
)
